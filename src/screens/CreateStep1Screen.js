...
import { View as RNView, Steps, FormList, FormItem, Loading, Picker, CountDown } from '../components';
import { AuthenticatedUserContext } from '../providers';
import { useWeather, useUserTemplateList, getTemplate, getPosition } from '../hooks/useData';
...
export const CreateStep1Screen = React.memo(({ route, navigation }) => {
  ...
  const loading = React.useMemo(() => {
    return !(weatherData && templateData && positionData && currentUserTemplate);
  ...
  return (
    <ScrollView style={{ flex: 1 }}>
      ...
      <FormList>
        ...
        <FormItem label='监测点位'><Text text16>{positionData?.name}</Text></FormItem>
        <FormItem label='树种' required>
          {treeSeedList && <Picker height={50} unstyle placeholder='请选择树种' value={formData.treeId} options={treeSeedList} name='treeId' onChange={value => handleChange('treeId', value)} />}</FormItem>
        <FormItem label='物候' required>{treeTypeList && <Picker height={50} unstyle placeholder='请选择物候' value={formData.phenology} options={treeTypeList} name='phenology' onChange={value => handleChange('phenology', value)} />}</FormItem>
        ...
        <FormItem label='监测时间'><CountDown onChange={recordTimeChange} /></FormItem>
        <FormItem label='测报人'><Text text16>{user.nickName}</Text></FormItem>
      </FormList>
      <View paddingV-40 paddingH-16>
        <Button label='下一步' borderRadius={4} style={{ height: 48 }} disabled={checkNext || confirmLoading} backgroundColor={Colors.primary} onPress={handleNext}></Button>
      </View>
    </ScrollView>
  );
})
