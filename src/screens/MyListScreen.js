...
export const MyListScreen = ({navigation}) => {
  ...
  const memoUserTemplateList = React.useMemo(() => {
    return (userTemplateList || []).filter(item => {
      ...
      if (query.bugId && item.bugId * 1 !== query.bugId * 1) {
        ...
        if (
          new Date(startDate).getTime() > new Date(item.recordTime) ||
          new Date(endDate).getTime() < new Date(item.recordTime)
        ) {
          flag = false;
        }
        ...
        query.endMonitorTime &&
        new Date(query.endMonitorTime).getTime() > item.recordTime
      ) {
        flag = false;
      }
      return flag;
  ...
  const makeData = React.useMemo(() => {
    ...
    return [
      ...memoUserTemplateList.map((item, index) => ({
        ...item,
  ...
  const uploadUserTemplate = async () => {
    await auth();
    const uploadList = memoUserTemplateList
      .map(item => ({...item, status: checkUserTemplateItem(item) ? '0' : '1'}))
    ...
    try {
      const res = await removeAll(
        uploadList.map(item => {
          return {
            ...item,
            templateCellDataMap: makeTableCellData(
              item._tableDataIndexMap,
              item._tableData,
              item._tableDataSize,
            ), // [{index:1, cell-3:1,
          };
