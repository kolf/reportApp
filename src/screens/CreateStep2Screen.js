...
import {makeTableCellData, makeTableAvg, makeTableSum} from '../lib/makeData';
...
export const CreateStep2Screen = React.memo(({route, navigation}) => {
    ...
    const run = async () => {
      try {
        updateUserTemplate({
          ...
          monitorAvg: makeTableAvg(memoData),
          monitorSum: makeTableSum(memoData),
  ...
  return (
    <ScrollView enableOnAndroid={true}>
      <View paddingV-20>
        <Steps items={stepList} current={1} />
      </View>
      <TableForm
        columns={columns}
        dataSource={memoData}
        rowKey="index"
        onChange={value => handleChange('tableData', value)}
      />

      <View paddingV-12 paddingH-16 row>
